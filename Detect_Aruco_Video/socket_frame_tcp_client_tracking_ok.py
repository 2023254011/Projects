...
def onMouse(event, x, y, flags, parm):
    global conn, mouse_rectangle, col, row, frame
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        #x=150q
        #y=100
        sendData = pack('IIII', col, row, x, y)
        print(col, row, x, y)
        conn.sendall(sendData)  # 클라이언트에게 응답
...
while True:
    ...
    # if you want to reset bounding box, select the 'r' key 
        ...
        trackers.clear()
        trackers = cv2.MultiTracker_create()
        box = cv2.selectROIs("TCP_Frame_Socket", frame, fromCenter=False, showCrosshair=True)
        print('box1',box)
        box = tuple(map(tuple, box))
        print('box2',box)
        for bb in box:
